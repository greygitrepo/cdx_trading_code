...
def compute_stops(
    entry: float, tp: float, sl: float, trail: float, side_long: bool
) -> Stops:
    if side_long:
        return Stops(
            stop_loss=entry * (1 - sl), take_profit=entry * (1 + tp), trailing=trail
        )
    return Stops(
        stop_loss=entry * (1 + sl), take_profit=entry * (1 - tp), trailing=trail
    )
...
def daily_loss_gate(
    equity_start: float, equity_now: float, daily_max_loss: float
) -> bool:
