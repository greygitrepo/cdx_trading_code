...
class RegimeDetector:
    ...
    def check_pause(self, z_1s: float, spread_mult: float, depth_drop: float, oi_drop_pct: float, ws_drop_rate: float) -> bool:
        triggers = 0
        ...
        triggers += 1 if (oi_drop_pct >= self.p.oi_drop_pct and ws_drop_rate > 0.0) else 0
        self.paused = triggers >= 2 or self.paused
        return self.paused

    def check_resume(self, rv_z: float, spread_mult: float, depth_recover: float) -> bool:
        if self.paused and rv_z <= self.p.resume_rv_z and spread_mult <= self.p.resume_spread_mult and depth_recover >= self.p.resume_depth_recover:
            self.paused = False
