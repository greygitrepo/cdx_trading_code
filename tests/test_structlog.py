...
def test_structlog_schema(tmp_path: Path) -> None:
    ...
    slog.log_order(
        ts=2,
        symbol="BTCUSDT",
        plan={"side": "BUY", "qty": 0.01, "price": 50000.0},
        result={"ok": True},
    )
    slog.log_fill(
        ts=3, symbol="BTCUSDT", side="BUY", price=50010.0, qty=0.01, order_id="oid-1"
    )
    slog.log_cancel(ts=4, symbol="BTCUSDT", order_link_id="ol-1", reason="test")
    slog.log_risk(
        ts=5, symbol="BTCUSDT", ok=False, reason="guard", context={"stage": "size"}
    )
