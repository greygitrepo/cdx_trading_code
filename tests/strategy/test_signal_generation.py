...
def test_signal_expected_on_known_window():
    from bot.core.strategies import StrategyParams, mis_signal, vrs_signal, select_strategy
    ...
    side_mis, score_mis = mis_signal(closes, orderbook_imbalance=0.70, spread=0.0003, spread_threshold=0.0005, params=params)
    side_vrs, score_vrs = vrs_signal(closes, vols, params)
    name, side = select_strategy((side_mis, score_mis), (side_vrs, score_vrs), (None, 0.0))
